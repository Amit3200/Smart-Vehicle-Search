    ...
    while(True):
        try:
            ...
            if s[0] in d or s[1] in d:
                ...
                msg="Your Vehicle with the number plate "+d+" Found at Jaipur Toll Tax. Contact Jaipur Toll Tax Soon"
                ...
                cur=db.cursor()
                num="'"+str(number)+"'"
                query="update user set status='Found' where vehicle="+str(num);
                print(query)
                cur.execute(query)
                db.commit()
                db.close()
